...
gplvm_data = pd.read_csv(gplvm_data_path)
...
# scaled_df = MinMaxScaler(feature_range=(0, 1)).fit_transform(features)

# # Joining the columns back on
# scaled_df = pd.DataFrame(scaled_df, columns=features.columns)

# joined_df = pd.concat([scaled_df, pca_data[["pca_dim0", "pca_dim1"]], gplvm_data[["gplvm_dim_0", "gplvm_dim_1"]]], axis=1)
...
# corr_gplvm_dim0 = corr["gplvm_dim_0"].sort_values(ascending=False)
# corr_gplvm_dim1 = corr["gplvm_dim_1"].sort_values(ascending=False)
